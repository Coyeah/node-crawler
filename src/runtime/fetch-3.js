...
(async () => {
  ...
  for (let i = 20000; i < data.length; i++) {
    ...
    if (i % 1000 == 0 && i != 9000) {
      await fs.writeFile(`${route}/detialData-${3}-${Date.now()}.json`, JSON.stringify(outData), (err) => {
        if(err) {
          return console.log(err);
        }
        console.log(chalk.red('MAKE >>>') + `${route}/detialData-${3}-${Date.now()}.json was saved!`);
      });
